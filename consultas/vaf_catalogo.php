    ...
    if(!isset($fila)){
        ...
        if ($query->rowCount() > 0) {
            foreach ($results as $prod) { 
                ...
                $prod->foto_producto == '' || $prod->foto_producto == '.'
                ? $foto = './consultas/docs/img/sin_imagen.png'
                : $foto = './consultas/docs/img_productos/'.$prod->foto_producto;

                echo "<tr {$class}>
                    <td id='tdId".$prod->id_producto."' class='fotoProducto'><img src='{$foto}'></td>
                    <td id='tdId".$prod->id_producto."'>{$prod->codigo_producto}</td>
                    <td id='tdId".$prod->id_producto."'>{$prod->nombre_producto}</td>
                    <td id='tdId".$prod->id_producto."'>{$prod->descripcion}</td>
                    <td id='tdId".$prod->id_producto."'>$ {$prod->precio_producto},00</td>";
                    ...
                    for (let i = 1; i <= 3; i++) {
                        const row = document.getElementById("tdId'.$prod->id_producto.'");
                        row.addEventListener("click", () => {
                          window.location.href = "?lnk=detalle&codigo='.$prod->codigo_interno.'";
                        });
                      }
